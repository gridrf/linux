...
static int command_to_op_code(char *reg_op_cmd)
{
	int op_code;
	int i;

	for (i = 0; i < REG_OP_STRLEN; i++)
		reg_op_cmd[i] = toupper(reg_op_cmd[i]);
			     ...
			     struct iio_regmap_op *reg_op, unsigned int line)
{
	...
	memset(reg_op_cmd, 0, REG_OP_STRLEN);
	strncpy(reg_op_cmd, fw_reg_cmd, REG_OP_STRLEN);

	reg_op->op = command_to_op_code(reg_op_cmd);
	if (reg_op->op < 0) {
...
static int iio_run_regmap_ops(struct device *dev, struct regmap *regmap,
			      ...
			      unsigned int nr_ops)
{
	const struct iio_regmap_op *reg_op;
	int ret = 0;
	int i;
